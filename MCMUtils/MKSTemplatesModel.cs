...
namespace WildBlueIndustries
{
    ...
    class MKSTemplatesModel
    {
        ...
        {
            string value;
            List<MKSModuleController> mksModSwitchers;
            ...
            if (this.part.vessel != null)
            {
                foreach (Part part in this.vessel.parts)
                {
                    if (part != this.part)
                    {
                        ...
                        mksModSwitchers = part.FindModulesImplementing<MKSModuleController>();
                        if (mksModSwitchers != null)
                            foreach (MKSModuleController switcher in mksModSwitchers)
                                if (switcher.shortName == "Fabricator" || switcher.shortName == "Modular Factory")
                                    hasFabricatorOrFactory = true;

                        //TODO: check legacy MKS modules
                    }
                }
